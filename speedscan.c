...
#define REQUEST_DELAY_MICROSECONDS 10000000 // 5 seconds
...
void perform_request(const char *url, const char *path) {
    // Check default response if not set
    if (defaultResponse == 0) {
        getVerifiedResponse(url, "fakepath12345", &defaultResponse, NULL, NULL);
    }
    ...
    if (status == defaultResponse) {
        falsePositivesCount++;
        if (falsePositivesCount > MAX_FALSE_POSITIVES) {
            fprintf(stdout, "False Positive detected for URL: %s/%s\n", url, path);
            falsePositivesCount = 0; // Reset count
        }
        return;
    }
    ...
    if (status == 200 || status == 204 || status == 300 || status == 301 || status == 302 || status == 307 || status == 401 || status == 403 || status == 405 || status == 500) {
...
int getVerifiedResponse(const char *url, const char *path, long *status, size_t *content_length, int *dir) {
    ...
    if (dir) {
        ...
        if (bodyfile) {
            ...
            curl_easy_setopt(curl, CURLOPT_NOBODY, 0L); // Fetch body
